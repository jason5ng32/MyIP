...
function transformDataFromIPapi2(data,ipGeoSource,vm) {
    if (data.error) {
        throw new Error(data.reason);
    }
    ...
    if (ipGeoSource === 0) {
        const proxyDetails = extractProxyDetails2(data.proxyDetect,vm);
        return {
            ...baseData,
            ...proxyDetails,
        };
...
// 提取代理信息
function extractProxyDetails2(proxyDetect = {},vm) {
    const isProxy = proxyDetect.proxy === 'yes' ? vm.$t('ipInfos.proxyDetect.yes') :
        proxyDetect.proxy === 'no' ? vm.$t('ipInfos.proxyDetect.no') :
            vm.$t('ipInfos.proxyDetect.unknownProxyType');
    const type = proxyDetect.type === 'Business' ? vm.$t('ipInfos.proxyDetect.type.Business') :
...
export { transformDataFromIPapi, extractProxyDetails,transformDataFromIPapi2,extractProxyDetails2};